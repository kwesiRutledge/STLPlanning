"""
straight_benchmark-2.py
Description:
    Creates code for the second benchmark problem.
    It involves a single agent moving through a single block obstacle.
"""
...
def create_optimistic_context():
    ...
    b1 = np.array([-3.0, 3.5, -1.75, 2.25], dtype = np.float64)
    goals = [(A, b1)]
...
def create_real_context(plot_tuples):
    ...
    b = np.array([-1.6, 2.2, -1.75, 2.25], dtype = np.float64)
    plot_tuples1.append(
        [ [(A,b)] , 'm' ]
    )
...
if __name__ == '__main__':
    ...
    fig1.savefig("benchmark2-straight_clear.png")

    plots1, specs_real = create_real_context(plots0)

    fig2 = plot_plan([pwl_curve0],plots1)
    fig2.savefig("benchmark2-straight_obstacle.png")
    ...
    # Save Results
    with open('benchmark2-results.yml','w') as outfile:
